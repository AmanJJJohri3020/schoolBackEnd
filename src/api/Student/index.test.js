import request from 'supertest'
import { apiRoot } from '../../config'
import { signSync } from '../../services/jwt'
import express from '../../services/express'
import { User } from '../user'
import routes, { Student } from '.'

const app = () => express(apiRoot, routes)

let userSession, anotherSession, student

beforeEach(async () => {
  const user = await User.create({ email: 'a@a.com', password: '123456' })
  const anotherUser = await User.create({ email: 'b@b.com', password: '123456' })
  userSession = signSync(user.id)
  anotherSession = signSync(anotherUser.id)
  student = await Student.create({ creator: user })
})

test('POST /Students 201 (user)', async () => {
  const { status, body } = await request(app())
    .post(`${apiRoot}`)
    .send({ access_token: userSession, title: 'test', content: 'test' })
  expect(status).toBe(201)
  expect(typeof body).toEqual('object')
  expect(body.title).toEqual('test')
  expect(body.content).toEqual('test')
  expect(typeof body.creator).toEqual('object')
})

test('POST /Students 401', async () => {
  const { status } = await request(app())
    .post(`${apiRoot}`)
  expect(status).toBe(401)
})

test('GET /Students 200', async () => {
  const { status, body } = await request(app())
    .get(`${apiRoot}`)
  expect(status).toBe(200)
  expect(Array.isArray(body)).toBe(true)
})

test('GET /Students/:id 200', async () => {
  const { status, body } = await request(app())
    .get(`${apiRoot}/${student.id}`)
  expect(status).toBe(200)
  expect(typeof body).toEqual('object')
  expect(body.id).toEqual(student.id)
})

test('GET /Students/:id 404', async () => {
  const { status } = await request(app())
    .get(apiRoot + '/123456789098765432123456')
  expect(status).toBe(404)
})

test('PUT /Students/:id 200 (user)', async () => {
  const { status, body } = await request(app())
    .put(`${apiRoot}/${student.id}`)
    .send({ access_token: userSession, title: 'test', content: 'test' })
  expect(status).toBe(200)
  expect(typeof body).toEqual('object')
  expect(body.id).toEqual(student.id)
  expect(body.title).toEqual('test')
  expect(body.content).toEqual('test')
  expect(typeof body.creator).toEqual('object')
})

test('PUT /Students/:id 401 (user) - another user', async () => {
  const { status } = await request(app())
    .put(`${apiRoot}/${student.id}`)
    .send({ access_token: anotherSession, title: 'test', content: 'test' })
  expect(status).toBe(401)
})

test('PUT /Students/:id 401', async () => {
  const { status } = await request(app())
    .put(`${apiRoot}/${student.id}`)
  expect(status).toBe(401)
})

test('PUT /Students/:id 404 (user)', async () => {
  const { status } = await request(app())
    .put(apiRoot + '/123456789098765432123456')
    .send({ access_token: anotherSession, title: 'test', content: 'test' })
  expect(status).toBe(404)
})

test('DELETE /Students/:id 204 (user)', async () => {
  const { status } = await request(app())
    .delete(`${apiRoot}/${student.id}`)
    .query({ access_token: userSession })
  expect(status).toBe(204)
})

test('DELETE /Students/:id 401 (user) - another user', async () => {
  const { status } = await request(app())
    .delete(`${apiRoot}/${student.id}`)
    .send({ access_token: anotherSession })
  expect(status).toBe(401)
})

test('DELETE /Students/:id 401', async () => {
  const { status } = await request(app())
    .delete(`${apiRoot}/${student.id}`)
  expect(status).toBe(401)
})

test('DELETE /Students/:id 404 (user)', async () => {
  const { status } = await request(app())
    .delete(apiRoot + '/123456789098765432123456')
    .query({ access_token: anotherSession })
  expect(status).toBe(404)
})
