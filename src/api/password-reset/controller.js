...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your SkulBackEnd account.<br>
        ...
        &mdash; SkulBackEnd Team
      `
      return sendMail({ toEmail: email, subject: 'SkulBackEnd - Password Reset', content })
    })
